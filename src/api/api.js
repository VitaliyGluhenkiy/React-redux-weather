...
export const weatherValueAPI = {
    getFirstValue(){
        return axios.get(`${api.base}?city=Brovary?&key=${api.key}`)
            .then(data => data.data)
    },
    getWeatherValue(query) {
        return axios.get(`${api.base}?city=${query}?&key=${api.key}`)
            .then(data => data.data)
    }
}
...
export const weatherForecastAPI = {
    getForecastTenDays(query) {
        return axios.get(`${api.baseDaily}?city=${query}?&key=${api.key}&days=5`)
            .then(data => data)
    }
}